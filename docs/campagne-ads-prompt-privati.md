...
> **Materiale già prodotto con questi prompt.** Il primo giro sta in
> [`campagne-ads-privati-materiale-nuovo.md`](campagne-ads-privati-materiale-nuovo.md):
> cinque varianti di copy, sei testi di rimarketing, un carosello, tre reel e il modulo
> istantaneo. Prima di rieseguire un prompt, guarda lì cosa esiste già.
...
Il carosello ha il costo per conversione più basso sulla lead generation. Per i privati in
[`creativi/caroselli/`](creativi/caroselli/) ce ne sono due: `facebook` (quattro carte,
minaccia → posta in gioco → soluzione → CTA) e `stanze` (cinque carte, il giro della casa),
prodotta con questo stesso prompt. C'è poi il carosello **miti** sui luoghi comuni, che sta
invece in [`creativi/formati/`](creativi/formati/). Questo prompt serve a produrre **una
sequenza ancora diversa** già nel formato che la vostra pipeline mangia: prima di usarlo,
scegliete un tema che non sia né il rischio né il giro di casa.
...
FORMATO DELLA RISPOSTA
Una voce JavaScript pronta da aggiungere all'oggetto CONTENUTI di
docs/creativi/caroselli/genera-caroselli.mjs, che ha esattamente questa forma:

  stanze: {
    etichetta: 'Check-up della rete di casa',
    carte: [
      { fondo: 'nero', foto: 'privati-router.jpg', occhiello: '...', titolo: '...',
        testo: '...' },
      { fondo: 'bianco', occhiello: '...', titolo: '...',
        elenco: ['...', '...', '...'] },
      { fondo: 'bianco', occhiello: '...', titolo: '...', testo: '...' },
      { fondo: 'bianco', occhiello: '...', titolo: '...', elenco: ['...', '...', '...'] },
      { fondo: 'nero', foto: 'privati-telecamera.jpg', occhiello: '...', titolo: '...',
        testo: '...', cta: '...', piede: 'Tecnolink · Firenze e dintorni' },
    ],
  },

Regole del formato — se non le rispetti il generatore produce carte sbagliate:
- `fondo` vale 'nero' o 'bianco'. Le carte nere aprono e chiudono la sequenza, quelle in
  mezzo sono bianche.
- `foto` sta solo sulle carte nere e può essere soltanto uno dei file già presenti in
  tecnolinkSite/wwwroot/img/campagne/foto/ — per i privati: privati-router.jpg e
  privati-telecamera.jpg. Se una carta chiederebbe una foto che non esiste, lascia fuori
  il campo e segnalalo a parte: le foto nuove si producono con i prompt del §2.
- Ogni carta ha `occhiello` (2-4 parole) e `titolo`. Il corpo è `testo` (una frase) oppure
  `elenco` (3-4 voci corte), mai tutti e due.
- Solo l'ultima carta ha `cta` e `piede`.
- La chiave in cima ('stanze') finisce nel nome dei PNG generati: una parola sola,
  minuscola, senza accenti.

Sotto l'oggetto, in italiano: due righe che spiegano contro quale carosello esistente va
messo a confronto e con quale utm_content (quello del carosello attuale è `carosello-bn`).
```
